...
extension PositionExtension on NodePosition {
  //NOTE: we need to take care about VIM mode
  NodePosition? moveHorizontal(
    Node root, {
    bool forward = true,
    SelectionRange selectionRange = SelectionRange.character,
  }) {
    final Node? node = root.queryPath(path);
    if (node == null) {
      return null;
    }
  ...
  // I take this from appflowy. I'll modify this later
  // when EasyEditorState be implemented and working as expected
  // NodePosition? moveVertical(
  ...
  //       );
  //       return NodePosition(path: neighbourPath, offset: offset);
  //     }
  //   }
  //
  ...
  //   if (delta != null) {
  //     if (upwards) {
  //       return NodePosition(path: path, offset: 0);
  //     } else {
  //       final length = delta.length;
  //       // move the cursor to the end of the node
  //       return NodePosition(path: path, offset: length);
  //     }
  //   }
  //
